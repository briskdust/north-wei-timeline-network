...
// 数据库字段到前端类型的转换函数
const convertDbPersonToFrontend = (dbPerson: any): Person => ({
  id: dbPerson.id,
  name: dbPerson.name,
...
const convertDbTimelineToFrontend = (dbEvent: any): TimelineEvent => ({
  id: dbEvent.id,
...
const convertDbRelationshipToFrontend = (dbRel: any): Relationship => ({
  id: dbRel.id,
  person1Id: dbRel.person1_id,
  person2Id: dbRel.person2_id,
  type: dbRel.type as Relationship["type"],
...
export const getAllPersons = async (): Promise<Person[]> => {
  try {
    const data = await apiRequest<any[]>("/people");
    return data.map(convertDbPersonToFrontend);
  } catch (error) {
...
export const getPersonById = async (
  ...
  try {
    const data = await apiRequest<any>(`/people/${id}`);
...
export const getPersonTimeline = async (
  ...
  try {
    const data = await apiRequest<any[]>(`/people/${personId}/timeline`);
...
export const getPersonRelationships = async (
  ...
  try {
    const data = await apiRequest<any[]>(`/people/${personId}/relationships`);
...
export const getPeopleByDynasty = async (
  ...
  try {
    const data = await apiRequest<any[]>(`/dynasties/${dynasty}/people`);
    return data.map(convertDbPersonToFrontend);
  } catch (error) {
...
export const getStatistics = async (): Promise<{
  ...
  try {
    const data = await apiRequest<any>("/stats");
    return data;
  } catch (error) {
